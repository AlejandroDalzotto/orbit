...
export default function AccountList() {
  const { accounts, error, isLoading } = useWalletAccounts()
  ...
  const sortedAccounts = Array.isArray(accounts) ? [...accounts].sort((a, b) => {
    switch (sortBy) {
      case "balance":
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        {sortedAccounts !== null ? sortedAccounts.map((account, index) => (

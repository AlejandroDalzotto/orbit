...
export default function ModalTransactionInformation({ transaction }: CardProps) {
  ...
  return (
    <div className="relative p-6 text-sm rounded shadow-lg bg-black border border-neutral-700 w-lg overflow-y-auto">
      ...
      <div className="flex items-center space-x-2 mb-4">
        <h2 className="text-lg font-semibold truncate">{transaction.details}</h2>
        <span className="px-2 text-sm rounded border border-neutral-700 lowercase text-neutral-500">
          {transaction.category}
        </span>
      </div>
      <p className="mb-4 font-light text-white text-2xl">${transaction.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
      {transaction.category === 'supermarket' && (
        <p className="mb-4 text-neutral-500">{transaction.storeName}</p>
      )}
      {transaction.category === 'supermarket' && transaction.items && (
      ...
      <div className="flex items-center justify-around border-t border-neutral-600 pt-4">
        <p className="mb-4 text-neutral-500">created at: {new Date(transaction.createdAt).toLocaleDateString('es-AR')}</p>
        <p className="mb-4 text-neutral-500">last time updated: {new Date(transaction.updatedAt).toLocaleDateString('es-AR')}</p>
      </div>
    </div>
  )
}

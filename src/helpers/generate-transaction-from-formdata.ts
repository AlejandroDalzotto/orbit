...
type CategoryFields = {
  ...
  [TransactionCategory.Freelance]: Pick<FreelanceTransaction, "client" | "project">;
  [TransactionCategory.Salary]: Pick<SalaryTransaction, "employer" | "extraDetails" | "job">;
  [TransactionCategory.Supermarket]: Pick<SupermarketTransaction, "items" | "storeName">;
};
...
export function buildTransactionFromFormData<T extends "create" | "edit">(
...
): T extends "create" ? RequestCreateTransaction : RequestEditTransaction {
  ...
  return { ...result, ...extraFields } as T extends "create" ? RequestCreateTransaction : RequestEditTransaction;
}